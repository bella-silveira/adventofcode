...
const findJoltPermutation = () => {
  ...
  for(let i = 1; i < sortedAdapters.length; i++) {
    const permutationSum = (permutations[sortedAdapters[i] - 1] || 0) + (permutations[sortedAdapters[i] - 2] || 0) + (permutations[sortedAdapters[i] - 3] || 0)
    
    permutations[sortedAdapters[i]] = permutationSum
  }
